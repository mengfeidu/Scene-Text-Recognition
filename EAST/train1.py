...
#	scheduler = lr_scheduler.CosineAnnealingLR(optimizer,200,0)
	...
	all_loss = []
	current_i = 0
	for epoch in range(1000,epoch_iter):	
...
#			lr_now = scheduler.get_last_lr()
			progress_bar(40,loss.item(),current_i,whole_number,lr)
		scheduler.step()
		print('epoch_loss is {:.8f}, epoch_time is {:.8f}'.format(epoch_loss/int(file_num/batch_size), time.time()-epoch_time))
		all_loss.append(epoch_loss/int(file_num/batch_size))
		print(time.asctime(time.localtime(time.time())))
		plt.plot(all_loss)
		plt.savefig('loss_landscape6.png')
		plt.close()
		print('='*50)
		if (epoch + 1) % interval == 0:
			state_dict = model.module.state_dict() if data_parallel else model.state_dict()
			torch.save(state_dict, os.path.join(pths_path, 'model_epoch_{}.pth'.format(epoch+1)))
			output = open(os.path.join(pths_path, 'loss1_1200.pkl'), 'wb')
...
if __name__ == '__main__':
	...
	train_img_path = '/root/dmf/dataset/train/img/'
	train_gt_path  = '/root/dmf/dataset/train/gt/'
	pths_path      = './pths'
	...
	save_interval  = 5
	eval_interval  = 5
	train(train_img_path, train_gt_path, pths_path, batch_size, lr, num_workers, epoch_iter, save_interval,checkpoint)
